...
export class DetailProductComponent implements OnInit {
    ...
    constructor(private httpService: HttpServiceService, private route: ActivatedRoute, private router: Router, private store : Store) { }

    ngOnInit(): void {
        let id : number = Number(this.route.snapshot.paramMap.get('id'));
        this.product = this.httpService.getProductById(id);
        setTimeout(() => {
            this.product.subscribe((value) => {if(!value) this.noProduct()})
        }, 5000)
    }
